...
class HistoryAdapter : ListAdapter<DriverStats, HistoryAdapter.ViewHolder>(DiffCallback()) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        Glide.with(holder.imgSnapshot).clear(holder.imgSnapshot)
        holder.imgSnapshot.setImageResource(R.drawable.ic_person)

        when {
            item.cloudImageUrl.contains("/api/images/") -> loadProtectedImage(holder, item)
            item.cloudImageUrl.isNotEmpty() -> loadImage(holder, item.cloudImageUrl)
            item.localImagePath.isNotEmpty() -> loadImage(holder, File(item.localImagePath))
        }
        ...
        holder.tvSynced.text = if (item.isSynced) "• Đã sao lưu server" else "• Chưa sao lưu"
        holder.tvSynced.setTextColor(
            ContextCompat.getColor(
                holder.itemView.context,
                if (item.isSynced) R.color.success else R.color.warning
            )
        )
    }

    private fun loadProtectedImage(holder: ViewHolder, item: DriverStats) {
        val expectedId = item.id
        val user = FirebaseAuth.getInstance().currentUser ?: return
        user.getIdToken(false).addOnSuccessListener { result ->
            val currentPosition = holder.bindingAdapterPosition
            if (currentPosition == RecyclerView.NO_POSITION || getItem(currentPosition).id != expectedId) {
            ...
            loadImage(holder, glideUrl)
        }
    }

    private fun loadImage(holder: ViewHolder, source: Any) {
