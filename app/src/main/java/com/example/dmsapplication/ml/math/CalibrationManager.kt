...
class CalibrationManager(context: Context) {
    ...
    val isPnpCalibrated: Boolean
        get() = prefs.getBoolean(getFullKey(KEY_IS_PNP_CALIBRATED), false)
    ...
    fun pnpHeadScore(current: PnPHeadPoseEstimator.HeadAngles?): Float? {
        ...
        return max(
            abs(current.yawDegrees - pnpBaselineYaw) / PNP_YAW_THRESHOLD_DEGREES,
            abs(current.pitchDegrees - pnpBaselinePitch) / PNP_PITCH_THRESHOLD_DEGREES
        )
    }
}